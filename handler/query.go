...
// Query 查询手机号
func Query(s *models.AlertStore) string {
	// 获取告警组并去重
	var groups []string
...
func queryDB(group string) string {
	// 初始化数据库连接
	path := "settings.yml"
	database.Init(path)
	...
	// 执行原生 SQL 查询
	//start := time.Now()
	result := database.DB.Raw("SELECT phone FROM test.`phone` WHERE Name in (SELECT Name FROM test.`Group` WHERE Groupname = ?)", group).Scan(&receiver)
